...
def get_right_arrow_symbol():
    lines = []
    lines.append([np.array([0.0, -0.25]), np.array([0.0, 0.25])])
    lines.append([np.array([0.0, -0.25]), np.array([0.0, -0.25])])
    lines.append([np.array([0.0, -1.0]), np.array([0.0, 0.0])])
    return lines
...
def get_rotation_spring_symbol():
    ...
    for i in range(0, 50):
        ...
        lines.append([[f_n[0], s_n[0]], [f_n[1], s_n[1]]])
    return lines


def get_displacement_block():
    lines = []
    lines.append([np.array([0.0, -0.5]), np.array([0.0, -0.5])])
    lines.append([np.array([0.0, 0.5]), np.array([0.0, -0.5])])
    lines.append([np.array([-0.7, 0.7]), np.array([-0.5, -0.5])])
    return lines
...
def get_displacement_free_y():
    ...
    lines.append([np.array([0.0, -0.5]), np.array([0.0, 0.5])])
    lines.append([np.array([0.0, -0.5]), np.array([0.0, -0.5])])
    lines.append([np.array([-0.5, -0.5]), np.array([0.7, -0.7])])
    lines.append([np.array([-0.7, -0.7]), np.array([0.7, -0.7])])
    return lines
